...
class ProductAttributeSet(models.Model):
    ...
    attribute_ids = fields.Many2many('product.attribute', 'attribute_set_attribute_relation', 'attribute_set_id', 'attribute_id', string='Attributes')
...
class ProductTemplate(models.Model):
    ...
    def get_variant_groups(self):
        ...
        for var in self.attribute_line_ids:
            if var.attribute_id.use_in_product_attributes_table and var.attribute_id.category_id:
...
class ProductAttribute(models.Model):
    ...
    display_type = fields.Selection(selection_add=[('text', 'Textbox'), ('textarea','Text Area'),('yes_no','Yes / No')])
    ...
    use_in_filter = fields.Boolean(string='Use in Filter', default=True)
    use_in_product_attributes_table = fields.Boolean(string='Use in Product Attributes Table', default=True)
    attribute_line_desc_ids = fields.One2many('product.attribute_desc.lines', 'attribute_id', 'Desc Lines',order='sequence desc')
...
class ProductAttributeValues(models.Model):
    ...
    sequence = fields.Integer(default=1,help="The sequence field is used to define order in which the tax lines are applied.",related="attribute_id.sort_order")
    attribute_set_id = fields.Many2one('product.attribute.set',string="Attribute Set")
    value_ids = fields.Many2many('product.attribute.value', string='Attribute Values')    
    attribute_text = fields.Char("Text")
    display_type = fields.Selection([('radio', 'Radio'),('select', 'Select'), ('color', 'Color'), ('text', 'Textbox'), ('textarea','Text Area'),('yes_no','Yes / No')],related='attribute_id.display_type')
    attribute_textarea = fields.Html("TextArea")
